...
public class DisplayArrowEntity extends Entity {

    private final Player shooter;
    private final ParticleMenu.ColoredParticle particle;
    private boolean wasStuck;

    public DisplayArrowEntity(@Nullable Player shooter, ParticleMenu.ColoredParticle particle) {
    ...
    private void shoot(@NotNull Point from, @NotNull Point to, double power, double spread) {
        ...
        shooter.sendPacket(getVelocityPacket());
        setView(
    ...
    public void tick(long time) {
        ...
        while (travelled < total) {
            ...
            ParticlePacket packet = new ParticlePacket(particle.particle(), pos, Vec.ZERO, 0, 1);
            shooter.sendPacket(packet);
            travelled += step;
        }
    ...
    @Override
    public @NotNull Acquirable<? extends DisplayArrowEntity> acquirable() {
        return (Acquirable<? extends DisplayArrowEntity>) super.acquirable();
    }
}
